...
import { DataBasket, Products } from '../../Interface/MyInterface';
...
export class InputNumberComponent {
  ...
  plus(){
    ...
    this.basketData[(this.first as number)].products[(this.second as number)].quantity = (this.takes as number) + 1;

    // this is for sending to the server
    (this.products as Products).quantity = (this.takes  as number) + 1

    this.basketService.updateCarts((this.idBasket as number),(this.products as Products))
    .subscribe((x:any)=> console.log("На сервері відбулась зміна данних ", x))
  ...
  deleteProduct(){
    let minusDelProduct = this.basketData[(this.first as number)].products.filter((elem:any, id:number)=> id !== this.second )
